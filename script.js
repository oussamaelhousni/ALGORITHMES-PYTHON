...
const cleanedHistoryExtension = historyExtension
  ...
  .replace(
    /Un transistor produit un signal 0 ou 1 puis huit transistors du registre présentent le nombre binaire 10110010/g,
    "Illustration d’un transistor contrôlant un signal électrique 0 ou 1",
  );
...
document.querySelectorAll(".history-slide").forEach((slide, index) => {
  ...
  if (module11Headings[index])
    slide.querySelector("h2").innerHTML = module11Headings[index];
});

const harderModule11Quizzes = [
  {
    prompt: "Quelle valeur décimale représente 11010110₂ ?",
    choices: ["198", "214", "226"],
    answer: "b",
  },
  {
    prompt: "Quelle valeur décimale représente 101101₂ ?",
    choices: ["41", "43", "45"],
    answer: "c",
  },
  {
    prompt: "Que forme la suite ASCII 67 · 79 · 68 · 69 ?",
    choices: ["DATA", "CODE", "CPU"],
    answer: "b",
  },
  {
    prompt:
      "Une image 2 × 2 en RGB utilise 8 bits par canal. Combien de bits faut-il ?",
    choices: ["12 bits", "48 bits", "96 bits"],
    answer: "b",
  },
  {
    prompt:
      "Pourquoi un registre de 8 bits peut-il représenter 256 valeurs différentes ?",
    choices: [
      "Parce que 8 + 8 = 16",
      "Parce que 2⁸ = 256",
      "Parce qu’un bit vaut toujours 256",
    ],
    answer: "b",
  },
];
...
if (exercisesSlide) {
  exercisesSlide.querySelector(".exercise-intro").textContent =
    "Résous ces défis en détaillant ton raisonnement. Les réponses doivent expliquer la représentation des données.";
...
const programQuiz = document.querySelector(
  '.slide[data-title="Quiz · Programme"] .quiz',
);
if (programQuiz) {
  programQuiz.dataset.answer = "b";
  programQuiz.querySelector("p").textContent =
    "Tu saisis 10110₂ + 00111₂ dans une calculatrice qui affiche le résultat en décimal. Que se passe-t-il ?";
...
const applicationsQuiz = document.querySelector(
  '.slide[data-title="Applications"] .quiz',
);
if (applicationsQuiz) {
  applicationsQuiz.dataset.answer = "c";
  applicationsQuiz.querySelector("p").textContent =
    "Tu veux écrire, exécuter et corriger un programme Python. Quel outil est le plus adapté ?";
...
const computerQuiz = document.querySelector(
  '.slide[data-title="Ordinateur"] .quiz',
);
if (computerQuiz) {
...
document.querySelectorAll(".history-slide").forEach((slide, index) => {
  ...
  stepper.innerHTML = historyLabels
    .map(
      (label, step) =>
        `<span class="${step === Math.min(index, 3) ? "active" : ""}"><b>${step + 1}</b>${label}</span>`,
    )
    .join("");
...
const transistorLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.querySelector("h2")?.textContent.includes("registre"),
);
if (transistorLesson) {
  transistorLesson.dataset.title =
    "11.3 · Le transistor : interrupteur électronique";
  transistorLesson.querySelector("h2").innerHTML =
    "Le transistor :<br /><span>un interrupteur électronique</span>";
  ...
  if (note)
    note.innerHTML =
      "<b>À retenir</b><span>Un transistor agit comme un interrupteur rapide qui transforme le courant en signal 0/1.</span>";
  ...
  if (transistorQuiz) {
    ...
    transistorQuiz.querySelector("p").textContent =
      "Que représente un transistor lorsqu’il contrôle le courant ?";
    const choices = [
      "Un interrupteur rapide",
      "Un fichier permanent",
      "Une image numérique",
    ];
    transistorQuiz
      .querySelectorAll("[data-choice]")
...
const textEncodingLesson = [
  ...document.querySelectorAll(".history-slide"),
].find((slide) => slide.dataset.title?.includes("11.7"));
if (textEncodingLesson) {
  const image = textEncodingLesson.querySelector("img");
  if (image) {
    image.src = "assets/text-encoding-v2.png?v=20260905-generated";
    image.alt = "La lettre A transformée en nombre puis en code binaire";
  }
}

const soundVideoLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.dataset.title?.includes("11.9"),
);
if (soundVideoLesson) {
  const image = soundVideoLesson.querySelector("img");
  if (image) {
    image.src = "assets/sound-video-v2.png?v=20260905-generated";
    image.alt =
      "Une onde sonore et une suite d’images vidéo transformées en données numériques";
  }
}

const whyInventLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.classList.contains("history-hero"),
);
if (whyInventLesson) {
  const image = whyInventLesson.querySelector("img");
  if (image) {
    image.src = "assets/history-origins-v2.png?v=20260905-generated";
    image.alt =
      "Outils anciens pour compter, avec un boulier, des pierres et des marques de comptage";
  }
}

const transistorSwitchLesson = [
  ...document.querySelectorAll(".history-slide"),
].find((slide) =>
  slide.querySelector("h2")?.textContent.includes("interrupteur électronique"),
);
if (transistorSwitchLesson) {
  const image = transistorSwitchLesson.querySelector("img");
  if (image) {
    image.src = "assets/transistor-switch-v2.png?v=20260905-generated";
    image.alt =
      "Un transistor utilisé comme interrupteur électronique, avec un circuit éteint et un circuit allumé";
  }
}

const binaryLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.dataset.title?.includes("11.5"),
);
if (binaryLesson) {
  const image = binaryLesson.querySelector("img");
  if (image) {
    image.src = "assets/binary-states-v2.png?v=20260905-generated";
    image.alt =
      "Des interrupteurs à deux états transformés en séquence binaire et en signal numérique";
  }
}

const conversionLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.dataset.title?.includes("11.6"),
);
if (conversionLesson) {
  const image = conversionLesson.querySelector("img");
  if (image) {
    image.src = "assets/binary-conversion-v2.png?v=20260905-generated";
    image.alt =
      "Le nombre décimal 10 converti en séquence binaire 1010 puis reconverti";
  }
}

const imageLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.dataset.title?.includes("11.8"),
);
if (imageLesson) {
  const image = imageLesson.querySelector("img");
  if (image) {
    image.src = "assets/pixels-rgb.png?v=20260905-slide21";
    image.alt =
      "Une image décomposée en pixels puis en canaux rouge, vert et bleu";
  }
}

// Updated visuals requested for slides 17, 21 and 22.
const cpuLesson = [...document.querySelectorAll(".history-slide")].find(
  (slide) => slide.dataset.title?.includes("11.4"),
);
if (cpuLesson) {
  const image = cpuLesson.querySelector("img");
  if (image) {
    image.src = "assets/transistor-real-illustration.png?v=20260905-slide17";
    image.alt =
      "Real transistor and illustration of its electronic switch behavior";
    image.alt =
      "Puce de processeur représentant l’aboutissement du passage du transistor au CPU";
  }
}
...
if (soundVideoLesson) {
  const image = soundVideoLesson.querySelector("img");
  if (image) {
    image.src = "assets/sound-video-v3.png?v=20260905-slide22";
    image.alt =
      "Une onde sonore et une suite d’images vidéo transformées en données numériques";
  }
}
...
document.querySelectorAll(".quiz").forEach((quiz) => {
  ...
  quiz.querySelectorAll("[data-choice]").forEach((button) => {
    button.addEventListener("click", () => {
      quiz
        .querySelectorAll("[data-choice]")
        .forEach((choice) => choice.classList.remove("correct", "wrong"));
      const isCorrect = button.dataset.choice === correct;
      button.classList.add(isCorrect ? "correct" : "wrong");
      feedback.className = `feedback ${isCorrect ? "success" : "error"}`;
      feedback.textContent = isCorrect
        ? "✓ Bravo ! Ton raisonnement est correct."
        : "Pas encore. Reprends la conversion étape par étape.";
    });
  });
});
showSlide(0);
